...
public abstract class ChessGamePiece {
    ...
    protected ArrayList<String> calculateNorthWestMoves(ChessGameBoard board, int numMoves) {
        ...
        if (isPieceOnScreen()) {
            for (int i = 1; i < 8 && count < numMoves; i++) {
                if (isOnScreen(pieceRow - i, pieceColumn - i) && (board.getCell(pieceRow - i, pieceColumn - i).getPieceOnSquare() == null)) {
    ...
    public void showLegalMoves(ChessGameBoard board) {
        ...
        if (isPieceOnScreen()) {
            for (String locStr : possibleMoves) {
                ...
                {
                    if (isEnemy(board, row, col)) {
                        board.getCell(row, col).setBackground(Color.YELLOW);
                    } else {
    ...
    public boolean isEnemy(ChessGameBoard board, int row, int col) {
        ...
        ChessGamePiece enemyPiece = board.getCell(row, col).getPieceOnSquare() == null ? null : board.getCell(row, col).getPieceOnSquare();
        if (enemyPiece == null || this.getColorOfPiece() == ChessGamePiece.UNASSIGNED || enemyPiece.getColorOfPiece() == ChessGamePiece.UNASSIGNED) {
            return false;
        }
    ...
    public ArrayList<ChessGamePiece> getCurrentAttackers(ChessGameBoard board) {
        ...
        int enemyColor = (this.getColorOfPiece() == ChessGamePiece.BLACK) ? ChessGamePiece.WHITE : ChessGamePiece.BLACK;
        ...
        for (int i = 0; i < board.getCells().length; i++) {
            for (int j = 0; j < board.getCells()[0].length; j++) {
                ...
                if (currPiece != null && currPiece.getColorOfPiece() == enemyColor) {
