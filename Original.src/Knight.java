...
public class Knight extends ChessGamePiece {
    ...
    private ArrayList<String> calculateNorthMoves(ChessGameBoard board) {
        ...
        for (int i = 2; i >= -2; i -= 4) {
            for (int j = 1; j >= -1; j -= 2) {
                if (isOnScreen(pieceRow + i, pieceColumn + j) && (isEnemy(board, pieceRow + i, pieceColumn + j) || board.getCell(pieceRow + i, pieceColumn + j).getPieceOnSquare() == null)) {
    ...
    public ImageIcon createImageByPieceType() {
        return Utils.getImageIconByType(
                getColorOfPiece(),
